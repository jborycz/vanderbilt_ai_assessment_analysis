...
ai_data_stats_all_plot <- ggplot() + 
  geom_point(data = subset(ai_data_stats_all), 
             ...
             size = 6, position = "dodge", alpha = 0.5) + 
  geom_text_repel(data = subset(ai_data_stats_all, before_after=="After" & subject != "Total"), 
      mapping = aes(x = before_after, y = mean, group = subject, color = subject, label = n), fontface = "bold", size= 5, hjust = -2, show.legend = FALSE) + 
  geom_line(data = subset(ai_data_stats_all), 
            mapping = aes(x = before_after, y = mean, group = subject, color = subject),
            linewidth = 1, linetype = "dashed", position = "dodge", alpha = 0.5) +
  geom_point(data = subset(ai_data_stats_all, subject == "Total"), 
             mapping = aes(x = before_after, y = mean),
             size = 6, color = "black", position = "dodge") + 
  geom_text_repel(data = subset(ai_data_stats_all, before_after=="After" & subject == "Total"), 
                  mapping = aes(x = before_after, y = mean, group = subject, label = n), color="black", fontface = "bold", size= 5, hjust = -2, show.legend = FALSE) + 
  geom_line(data = subset(ai_data_stats_all, subject == "Total"), 
            mapping = aes(x = before_after, y = mean, group = subject),
            linewidth = 1, linetype = "solid", color = "black", position = "dodge", alpha = 1) +
  ...
  labs(y = "Frequency/likelihood of using AI tools for research related tasks\n (1 - Low frequency/likelihood, 5 - High frequency/likelihood)", 
